...
fn ensure_parent_directory(path: &str) -> Result<(), String> {
    if let Some(parent) = Path::new(path).parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("繝・ぅ繝ｬ繧ｯ繝医Μ菴懈・繧ｨ繝ｩ繝ｼ: {}", e))?;
    }
    Ok(())
}
...
pub fn export_data(conn: &Connection, raw_path: &str) -> Result<(), String> {
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&export_payload)
            .map_err(|e| format!("JSON逕滓・繧ｨ繝ｩ繝ｼ: {}", e))?,
    )
    .map_err(|e| {
        format!(
            "繝輔ぃ繧､繝ｫ譖ｸ縺崎ｾｼ縺ｿ繧ｨ繝ｩ繝ｼ ({}): {}",
            adjusted_path, e
        )
    })?;

    println!("繧ｨ繧ｯ繧ｹ繝昴・繝亥ｮ御ｺ・ {}", adjusted_path);
    Ok(())
}
...
pub fn export_github_config(conn: &Connection, raw_path: &str) -> Result<(), String> {
    ...
    println!(
        "GitHub Actions 逕ｨ險ｭ螳壹ヵ繧｡繧､繝ｫ繧貞・蜉帙＠縺ｾ縺励◆: {}",
        adjusted_path
    );
